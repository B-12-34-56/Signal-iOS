...
public class SignalAttachment: NSObject {
    ...
    /// Returns a consistent SHA256 hash of the original file data (if available), or normalized image data as fallback.
    public var deduplicationHash: String? {
        // Prefer hashing the original file data
        if let url = self.dataUrl, let data = try? Data(contentsOf: url) {
            return SHA256.hash(data: data).map { String(format: "%02x", $0) }.joined()
        }
        // Fallback: hash normalized image data
        if let image = self.image(), let pngData = image.pngData() {
            return SHA256.hash(data: pngData).map { String(format: "%02x", $0) }.joined()
        }
        // Fallback: hash raw data
        return SHA256.hash(data: self.data).map { String(format: "%02x", $0) }.joined()
    }
    /// Returns the file extension for deduplication (lowercased, or "bin" as fallback)
    public var deduplicationFileExtension: String {
        return (self.fileExtension?.lowercased() ?? "bin")
    }
    /// Returns the MIME type for deduplication
    public var deduplicationMimeType: String {
